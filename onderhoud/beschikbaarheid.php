        ...
        if (!$speler || !filter_var($speler['email'], FILTER_VALIDATE_EMAIL)) {
            ...
            try {
                ...
                if ($actie === 'afwijzen_met_mail') {
                    ...
                    $stmt->execute([$activiteitId, $deelnemerId]);
                } elseif ($actie === 'uitnodigen' && !$testModus) {
                    $stmt = $pdo->prepare('UPDATE activiteit_deelnemers SET uitnodiging_verstuurd_op = NOW() WHERE activiteit_id = ? AND deelnemer_id = ?');
                    $stmt->execute([$activiteitId, $deelnemerId]);
                }
                if ($testModus) {
...
if ($gekozenActiviteit) {
    if ($toonModus === 'toegelaten') {
        $stmt = $pdo->prepare("SELECT d.id, d.voornaam, d.achternaam, d.email, ad.status, ad.toegelaten, ad.uitnodiging_verstuurd_op, COALESCE(ad.instrument_id, di.instrument_id) AS instrument_id, ad.partij, i.naam AS instrument FROM activiteit_deelnemers ad JOIN deelnemers d ON d.id = ad.deelnemer_id LEFT JOIN (SELECT deelnemer_id, MIN(instrument_id) AS instrument_id FROM deelnemer_instrumenten GROUP BY deelnemer_id) di ON di.deelnemer_id = d.id LEFT JOIN instrumenten i ON i.id = COALESCE(ad.instrument_id, di.instrument_id) WHERE ad.activiteit_id = ? AND ad.status <> 'nee' AND ad.toegelaten = 1 ORDER BY CASE WHEN i.id IS NULL THEN 1 ELSE 0 END, CASE WHEN LOWER(TRIM(i.naam)) = 'pauken' THEN COALESCE((SELECT MIN(i2.id) FROM instrumenten i2 WHERE LOWER(TRIM(i2.naam)) LIKE 'trompet%'), i.id) ELSE i.id END, CASE WHEN LOWER(TRIM(i.naam)) = 'pauken' THEN 1 ELSE 0 END, CASE WHEN LOWER(TRIM(i.naam)) = 'viool' AND LOWER(COALESCE(ad.partij, '')) REGEXP '2' THEN 2 WHEN LOWER(TRIM(i.naam)) = 'viool' THEN 1 ELSE 0 END, CASE WHEN LOWER(COALESCE(ad.partij, '')) REGEXP 'concertmeester|aanvoerder' THEN 0 ELSE 1 END, CASE WHEN TRIM(COALESCE(ad.partij, '')) = '' THEN 1 ELSE 0 END, ad.partij, d.achternaam, d.voornaam");
    } else {
        $stmt = $pdo->prepare("SELECT d.id, d.voornaam, d.achternaam, d.email, ad.status, ad.toegelaten, ad.uitnodiging_verstuurd_op, COALESCE(ad.instrument_id, di.instrument_id) AS instrument_id, ad.partij, i.naam AS instrument FROM activiteit_deelnemers ad JOIN deelnemers d ON d.id = ad.deelnemer_id LEFT JOIN (SELECT deelnemer_id, MIN(instrument_id) AS instrument_id FROM deelnemer_instrumenten GROUP BY deelnemer_id) di ON di.deelnemer_id = d.id LEFT JOIN instrumenten i ON i.id = COALESCE(ad.instrument_id, di.instrument_id) WHERE ad.activiteit_id = ? AND ad.status <> 'nee' AND ad.status IN ('ja', 'misschien') ORDER BY CASE WHEN i.id IS NULL THEN 1 ELSE 0 END, CASE WHEN LOWER(TRIM(i.naam)) = 'pauken' THEN COALESCE((SELECT MIN(i2.id) FROM instrumenten i2 WHERE LOWER(TRIM(i2.naam)) LIKE 'trompet%'), i.id) ELSE i.id END, CASE WHEN LOWER(TRIM(i.naam)) = 'pauken' THEN 1 ELSE 0 END, CASE WHEN LOWER(TRIM(i.naam)) = 'viool' AND LOWER(COALESCE(ad.partij, '')) REGEXP '2' THEN 2 WHEN LOWER(TRIM(i.naam)) = 'viool' THEN 1 ELSE 0 END, CASE WHEN LOWER(COALESCE(ad.partij, '')) REGEXP 'concertmeester|aanvoerder' THEN 0 ELSE 1 END, CASE WHEN TRIM(COALESCE(ad.partij, '')) = '' THEN 1 ELSE 0 END, ad.partij, d.achternaam, d.voornaam");
    }
...
?>
<!DOCTYPE html>
<html lang="nl"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Beschikbaarheid</title><link href="/css/moz.css" rel="stylesheet" type="text/css"><style>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('.status-selectie').forEach(function (selectie) {
        selectie.addEventListener('change', function () {
            if (!window.confirm('Status wijzigen naar ' + selectie.value + ' en opslaan?')) {
                selectie.value = selectie.dataset.huidigeStatus;
                return;
            }
            var rij = selectie.closest('tr');
            var form = document.createElement('form');
            form.method = 'post';
            form.action = window.location.href;
            var velden = {
                actie: 'status_opslaan',
