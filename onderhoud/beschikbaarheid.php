...
if (isset($_POST['actie'], $_POST['deelnemer_id'], $_POST['activiteit_id'])) {
    ...
    if (in_array($actie, ['toelaten', 'afwijzen_zonder_mail'], true) && $status === 'ja') {
        $stmt = $pdo->prepare('UPDATE activiteit_deelnemers SET instrument_id = ?, partij = ?, status = ?, toegelaten = ? WHERE activiteit_id = ? AND deelnemer_id = ?');
        $stmt->execute([$instrumentId, $partij, $status, $actie === 'toelaten' ? 1 : 0, $activiteitId, $deelnemerId]);
        $melding = $actie === 'toelaten'
            ? 'Deelnemer toegelaten zonder e-mail.'
            : 'Deelnemer afgewezen zonder e-mail.';
    } elseif (in_array($actie, ['uitnodigen', 'afwijzen_met_mail'], true) && $status === 'misschien') {
    ...
    if (in_array($actie, ['uitnodigen', 'afwijzen_met_mail'], true) && $status === 'misschien') {
        ...
        if (!$speler || !filter_var($speler['email'], FILTER_VALIDATE_EMAIL)) {
            ...
            try {
                ...
                $melding = $testModus
                    ? 'Testmail alleen naar dirkjan@pellegrina.net verstuurd; toelating bijgewerkt.'
                    : ($bevestigen ? 'Uitnodiging' : 'Afwijzingsmail') . ' verstuurd met CC aan Dirkjan; toelating bijgewerkt.';
...
$afgewezenDeelnemers = array_map('intval', array_column(array_filter($spelers, static fn (array $speler): bool => in_array($speler['status'], ['ja', 'misschien'], true) && $speler['toegelaten'] !== null && (int) $speler['toegelaten'] === 0), 'id'));
